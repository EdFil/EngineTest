...
SDL_Texture* TextureManager::loadTexture(const std::string& textureFileName) {
    ...
    //     FileManager::instance()->fullPathForFile(textureFileName.c_str()); int width, height,
    //     channelsInFile;
    ...
    //     SDL_Surface* surface = SDL_CreateRGBSurfaceFrom(static_cast<void*>(imageData), width,
    //     height,
    //                                                     depth, pitch, rmask, gmask, bmask,
    //                                                     amask);
    //     if (surface == nullptr) {
    //         SDL_LogError(SDL_LOG_CATEGORY_RENDER, "Error creating surface for image %s with error
    //         %s",
    ...
    //     stbi_image_free(imageData);
    //     if (texture == nullptr) {
    //         SDL_LogError(SDL_LOG_CATEGORY_RENDER, "Error creating texture for image %s with error
    //         %s",
    //                      imageFullPath.c_str(), SDL_GetError());
    //     }
