...
std::vector<char> FileManager::loadFile(const char fileName[]) const
{
	...
	if (fileReader) {
		...
		if (fileReader) {
			SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION, "[FileManager] Read file %s", fullPath.c_str());
		} else {
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "[FileManager] Failed to read file %s", fullPath.c_str());
		}

		fileReader.close();
	} else {
		SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "[FileManager] Failed open file %s", fullPath.c_str());
	}
